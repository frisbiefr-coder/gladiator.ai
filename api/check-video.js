...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (status.status === 'COMPLETED') {
      const result = await fal.queue.result('fal-ai/minimax/hailuo-02/standard/image-to-video', { requestId });
      ...
      if (videoUrl && userId) {
        try {
          await fetch(`${SUPABASE_URL}/rest/v1/user_generations`, {
            ...
            body: JSON.stringify({ user_id: userId, prompt: 'img2video', type: 'img2video', url: videoUrl, status: 'done' })
          });
        } catch(e) {}
      }
